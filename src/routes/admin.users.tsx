...
type Profile = { id: string; full_name: string | null; email: string | null; phone: string | null; nationality: string | null; country: string | null; gender: string | null; created_at: string };
...
function AdminUsers() {
  ...
  return (
    <AdminLayout title="Users" subtitle={`${rows.length} registered users`} actions={
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[800px]">
            <thead className="bg-secondary/60 text-xs text-muted-foreground">
              <tr><th className="text-left px-5 py-3">Name</th><th className="text-left px-3 py-3">Email</th><th className="text-left px-3 py-3">Phone</th><th className="text-left px-3 py-3">Country</th><th className="text-left px-3 py-3">Joined</th><th className="text-left px-3 py-3">Role</th><th></th></tr>
            </thead>
            <tbody>
              {filtered.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-t border-border">
                    ...
                    <td className="px-3 py-3 text-muted-foreground">{p.country ?? "—"}</td>
                    <td className="px-3 py-3 text-muted-foreground">{new Date(p.created_at).toLocaleDateString()}</td>
                    <td className="px-3 py-3"><span className={`text-[10px] font-semibold px-2 py-1 rounded-full ${isAdmin ? "bg-brand-blue text-white" : "bg-secondary text-foreground"}`}>{isAdmin ? "Admin" : "User"}</span></td>
              ...
              {filtered.length === 0 && <tr><td colSpan={7} className="text-center py-10 text-muted-foreground">No users</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
