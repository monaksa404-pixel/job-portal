...
import {
  Bookmark, MapPin, Star, BadgeCheck, Briefcase, Clock, Users, CreditCard,
...
function JobDetailPage() {
  ...
  const initials = job.company_name.split(" ").map((w) => w[0]).slice(0, 2).join("").toUpperCase();
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-border p-5 lg:p-6">
            <div className="flex gap-4">
              <div className="w-20 h-20 rounded-xl bg-amber-400 flex items-center justify-center text-brand-navy font-extrabold text-2xl shrink-0">
                {initials}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h1 className="text-xl lg:text-2xl font-extrabold text-brand-navy">{job.title}</h1>
                    <div className="mt-1 flex items-center gap-1 text-sm font-semibold text-brand-navy">
                      {job.company_name}
                      {job.verified && <BadgeCheck className="w-4 h-4 text-brand-blue" />}
                    </div>
                  ...
                  <div className="text-right">
                    ...
                    <Bookmark className="w-5 h-5 text-muted-foreground inline-block mt-2" />
                  </div>
                </div>
            ...
            <div className="mt-5 grid grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <Info1 icon={Users} label="Vacancies" value={`Male: ${job.male_required} | Female: ${job.female_required}`} />
          ...
          <div className="bg-white rounded-2xl border border-border p-5 lg:p-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
              <Facility on={job.accommodation} icon={Home} label="Free Accommodation" color="text-emerald-600 bg-emerald-50" />
              <Facility on={job.food} icon={Utensils} label="Free Food" color="text-orange-500 bg-orange-50" />
              <Facility on={job.transport} icon={Bus} label="Free Transport" color="text-blue-600 bg-blue-50" />
              <Facility on={job.medical_insurance} icon={ShieldPlus} label="Medical Insurance" color="text-rose-500 bg-rose-50" />
              <Facility on={job.overtime} icon={Timer} label="Overtime Available" color="text-amber-500 bg-amber-50" />
            </div>
          </div>
        ...
        <aside className="space-y-4">
          <div className="bg-white rounded-2xl border border-border p-5">
            <h3 className="font-bold text-brand-navy mb-3">Vacancies</h3>
            <div className="space-y-2 text-sm">
              <div className="flex items-center justify-between">
                <span className="flex items-center gap-2 text-brand-navy">
                  <span className="w-7 h-7 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center">♂</span>
                  Male Required
                </span>
                <span className="font-bold">{job.male_required}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="flex items-center gap-2 text-brand-navy">
                  <span className="w-7 h-7 rounded-full bg-rose-100 text-rose-500 flex items-center justify-center">♀</span>
                  Female Required
                </span>
                <span className="font-bold">{job.female_required}</span>
              </div>
              <div className="border-t border-border pt-2 flex items-center justify-between">
                <span className="font-semibold text-brand-navy">Total Vacancies</span>
      ...
      <div className="fixed bottom-16 lg:bottom-0 inset-x-0 z-30 bg-white border-t border-border">
        <div className="container mx-auto max-w-7xl px-4 lg:px-6 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 lg:gap-3 w-full lg:w-auto justify-end">
            <button className="px-4 py-2.5 rounded-xl border border-border text-sm font-semibold flex items-center gap-2 text-brand-navy">
...
function Info1({ icon: Icon, label, value }: { icon: typeof Briefcase; label: string; value: string }) {
  return (
    <div className="bg-secondary/60 rounded-xl p-3 flex items-center gap-3">
      <div className="w-9 h-9 rounded-lg bg-white flex items-center justify-center text-brand-blue">
...
function Facility({ on, icon: Icon, label, color }: { on: boolean; icon: typeof Home; label: string; color: string }) {
  return (
    <div className={`flex flex-col items-center text-center gap-2 ${on ? "" : "opacity-40"}`}>
